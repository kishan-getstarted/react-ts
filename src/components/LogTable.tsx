...
const LogTable: React.FC<LogTableProps> = ({
  ...
  const renderTimestamp = useCallback((timestamp: string) => (
    <div className="log-timestamp">{formatTimestamp(timestamp)}</div>
  ), []);
  ...
  const highlightSearchTerm = (text: string, searchTerm: string): React.ReactNode => {
    ...
    return parts.map((part, i) => part.toLowerCase() === searchTerm.toLowerCase() ? <span key={i} className="highlight">{part}</span> : part
    );
  };
  // Render body column
  const renderBody = useCallback((body: string, index: number) => {
      ...
      >
        {visibleColumns.map((column) => (
          <div key={column} className={`log-cell log-cell-${column}`}>
            {columnRenderers[column as keyof typeof columnRenderers](log[column as keyof LogEntry], index)}
          </div>
        ))}
      </div>
    ));
